...
subgraph LOOP["For each Synced device in disabled tags"]
    ...
    H -- "NotFound" --> K
    H -- Yes --> K
    E -- No --> K["Set device:<br/>Status = NonSyncing<br/>CorporateIdentityID = ''<br/>CorporateIdentity = ''<br/>LastCorpIdentitySync = UtcNow"]
    ...
    TAGRE --> END
    TAG -- "Tag still disabled" --> TAGUPD["Update fresh device:<br/>Status = NonSyncing<br/>Clear CorpID fields"]
